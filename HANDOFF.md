...
**Último cambio: 2026-06-30.**
Actualización 2026-06-30 (solo datos): corrección retroactiva asignaciones (Nicolás Rojas→HELP, Igor San Martín→Consorcio), 2 alertas resueltas + 2 nuevas Zurich (activo 4 críticas / 11 importantes / 5 seguimiento), notas+contexto Zurich (alias interno "CORTEX"). Pendiente próxima sesión: hitos del Plan CORTEX MVP (fechas sin confirmar). Previas: surfacing `proyectos.notas` en UI (`82b32c4`) + actualización 2026-06-26, actualización 2026-06-25 (migración `09_proyectos_notas`), M7 (Adiciones v4: Hitos + Reportes + Presentación semanal, migración `08_hitos`), `57943c4` M6 + carga de datos v1.0, `cc9ef41` M5, `52b2b83` M1/M2/M3/M4.
